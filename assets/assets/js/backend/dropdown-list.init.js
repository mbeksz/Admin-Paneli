...
var pagesList = new List("pagesList", options).on("updated", function (list) {
...
xhttp.onload = function () {
    var json_records = JSON.parse(this.responseText);   
        Array.from(json_records).forEach(function (element) {
            ...
            pagesList.add({
            id: `<a href="stylist-edit.php?id=`+element.id+`" class="fw-medium link-primary">${element.id}</a>`,           
            name: element.name,    
            menuURL: `<a href="stylist-edit.php?id=`+element.id+`" class="fw-medium link-primary">${element.id}</a>`,           

            });
            pagesList.sort('id', { order: "asc" });
            refreshCallbacks();
        });
    pagesList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">#TB01</a>`);

    
}
xhttp.open("GET", "/Yonetici/api/stylist.php");
xhttp.send();

isCount = new DOMParser().parseFromString(
    pagesList.items.slice(-1)[0]._values.id,
    "text/html"
);
                ...
                Array.from(itemValues).forEach(function (x) {
                    ...
                    if (isdeleteid == itemId) {
                        document.getElementById("delete-record").addEventListener("click", function () {
                            pagesList.remove("id", isElem.outerHTML);
...
statusInput.passedElement.element.addEventListener('change', function (event) {
    ...
    pagesList.filter(function (data) {
