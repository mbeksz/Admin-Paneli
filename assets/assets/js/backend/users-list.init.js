...
xhttp.onload = function () {
    ...
    user_count = json_records.user_count
            document.getElementById('userCount').textContent = json_records.user_count;


        Array.from(json_records.users).forEach(function (element) {
            
            usersList.add({
            id: `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ${element.id}</a>`,
            user_name: element.name + " "+ element.surname,
            email_id: element.email,
            date: element.create_time,           
            status: element.phone
            });
            usersList.sort('id', { order: "desc" });
            refreshCallbacks();
        });
...
xhttp.open("GET", "/Yonetici/api/users.php");
xhttp.send();
