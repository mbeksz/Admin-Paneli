...
function getList($conn) {
    $sql = "SELECT id, title FROM headers";
...
function getDetail($conn, $id) {
    ...
    $sql = "SELECT * FROM headers WHERE id = ?";
...
$id = intval($_GET['id']);
    ...
    $sql = "UPDATE headers SET title=?, content=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssi", $data['page_title'], $data['content'], $id);

    // Sorguyu çalıştır ve sonucu kontrol et
    if ($stmt->execute()) {
        sendResponse(201, 'Header updated successfully');
    } else {
...
function createPage($conn) {
    ...
    $stmt = $conn->prepare("INSERT INTO headers (title, content) VALUES (?, ?)");
